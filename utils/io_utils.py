...
def log_graph(
    ...
    for i in Gc.nodes():
        if nodecolor == "feat" and "feat" in Gc.nodes[i]:
            feat = Gc.nodes[i]["feat"]
            num_classes = feat.shape[0] if hasattr(feat, 'shape') else feat.size()[0]
            if num_classes >= 10:
    ...
    if Gc.number_of_nodes() == 0:
        print(f"Warning: empty graph for {name}, skipping visualization")
        return
    if Gc.number_of_edges() == 0:
        print(f"Warning: no edges in graph for {name}, skipping visualization")
        return
    # remove_nodes = []
    # for u in Gc.nodes():
    #    if Gc
