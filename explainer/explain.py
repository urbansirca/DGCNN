...
class Explainer:
    ...
    def explain(
        ...
        if self.graph_mode:
            # Handle pred shape (num_samples, 1, num_classes) or (num_samples, num_classes)
            if self.pred.ndim == 3:
                pred_label = np.argmax(self.pred[graph_idx], axis=-1).item()
            else:
        ...
        if model == "grad":
            ...
            for epoch in range(self.args.num_epochs):
                ...
                if self.writer is not None:
                    ...
                    if epoch == 0:
                        if getattr(self.model, 'att', False):
                            # explain node
...
class ExplainModule(nn.Module):
    ...
    def _masked_adj(self):
        ...
        # NEW: Get adjacency from inside the model
        L = normalize_A(self.model.A)  # model.A is the learned adjacency --> mask laplacian, not adjacency
        masked_adj = L * sym_mask 
    ...
    def forward(self, node_idx, contrast_class=None, unconstrained=False, mask_features=True, marginalize=False):
        ...
        # Feature masking
        if not unconstrained and mask_features:
        ...
        if self.mask_act == "sigmoid":
            ...
            sym_mask = nn.functional.relu(self.mask)
        sym_mask = (sym_mask + sym_mask.t()) / 2
        sym_mask = sym_mask * self.diag_mask

        # Temporarily replace model.A
        original_A = self.model.A
        masked_A = original_A * sym_mask
        self.model.A = nn.Parameter(masked_A, requires_grad=False)

        # Standard forward
        ypred = self.model(x)

        # Restore
        self.model.A = original_A

        # Store for logging
        self.masked_adj = (original_A * sym_mask).detach().unsqueeze(0)
    ...
    def adj_feat_grad(self, node_idx, pred_label_node):
        ...
        if self.args.gpu:
            ...
            x = self.x
        # DGCNN only takes x, uses internal adjacency
        ypred = self.model(x)
        if self.graph_mode:
            logit = nn.Softmax(dim=1)(ypred)[0]
        else:
            logit = nn.Softmax(dim=0)(ypred[self.graph_idx, node_idx, :])
        logit = logit[pred_label_node]
        loss = -torch.log(logit)
        loss.backward()
        # Return adjacency gradient from model.A and feature gradient
